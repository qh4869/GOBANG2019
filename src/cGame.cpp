...
//有关禁手看文件夹的图片
int cGame::referee(cBoard oBoard, const int x, const int y)
{
	int result = 0;//裁判结果
	//--------------------------------------白棋判断（没有禁手）-------------------------------
...
//例如假活三
bool cGame::KeyPointCheck(cBoard oBoard, const int x, const int y, int FirstSame, int direction)
{
	int KeyX, KeyY;
	//先计算关键点的坐标
	switch (direction)
...
int cGame::Gaming()
{
	...
	else if (GameMode == PLAYERVSAI)
	{
		...
		while (1)
		{
			...
			{
				oAI.AIAlgorithm(oBoard, PlayerColor, AI, cGame::KeyPointCheck);// this指针问题
				oAI.AIAlgorithm(oBoard, PlayerColor, PLAYER, cGame::KeyPointCheck);
